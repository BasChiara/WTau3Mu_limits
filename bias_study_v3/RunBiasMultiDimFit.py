...
if args.mode == "fit_fixed":
    ...
    if not do_single_fit :
        ...
        cmd = f'combine -M MultiDimFit {args.input_datacard} -m {tau_mass} --freezeParameters Mtau -t {args.nFit} -n {fit_tag} --expectSignal {args.r_gen} --toysFile {toys_file_name} --algo singles --rMin -20 --rMax 20 --trackParameters r -s 3872998\n\n'
        if not args.dry_run : os.system(cmd)
        else : print(cmd)
        # move fit result
