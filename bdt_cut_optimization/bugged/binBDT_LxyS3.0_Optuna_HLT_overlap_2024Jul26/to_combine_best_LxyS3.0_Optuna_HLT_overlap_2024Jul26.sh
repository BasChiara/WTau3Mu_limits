...
if [[ $METHOD == 'AsymptoticLimits' ]]; then
    text2workspace.py ${DIR}/datacard_WTau3Mu_full20${YEAR}_${TAG}.txt
    combine -M $METHOD ${DIR}/datacard_WTau3Mu_full20${YEAR}_${TAG}.root -n .WTau3Mu_full${YEAR} -t -1 --cl $CL
fi
if [[ $METHOD == 'HybridNew' ]]; then
    ...
    # merge results
    hadd higgsCombine.WTau3Mu_full${YEAR}.HybridNew.mH120.root higgsCombine.WTau3Mu_full${YEAR}.HybridNew.mH120.quant*.root 
fi
