...
BASE_DIR="$CMSSW_BASE/src/WTau3Mu_limits/"
...
for CATEGORY in ${CATEGORY_LIST[@]}; do
    ...
    echo -e "----------------\n"
    echo 'TIME TO FIT'
    python3 $BASE_DIR/models/Tau3Mu_fitSB.py --plot_outdir $EOS_DIR --combine_dir $COMBINE_DIR -s $SIGNAL -d $DATA --category $CATEGORY -y $YEAR --tag $TAG --optim_bdt --save_ws -b dynamic --BDTmin 0.9900 --BDTmax 0.9910 --BDTstep 0.0005
    echo 'TIME TO CALCULATE LIMITS'
    # with AsymptoticLimits
    python3 $BASE_DIR/models/runBDTOptimCombine.py -i $COMBINE_DIR -o $EOS_DIR --scan_sensitivity input_combine/sensitivity_tree_bdt_scan_WTau3Mu_$CATEGORY${YEAR}_$TAG.root -d WTau3Mu_$CATEGORY$YEAR -n WTau3Mu_$CATEGORY$YEAR --BDTmin 0.9900 --BDTmax 0.9910 --BDTstep 0.0005 -s all 
    # with HybridNew
    python3 $BASE_DIR/models/runBDTOptimCombine.py -i $COMBINE_DIR -o $EOS_DIR --scan_sensitivity input_combine/sensitivity_tree_bdt_scan_WTau3Mu_$CATEGORY${YEAR}_$TAG.root -d WTau3Mu_$CATEGORY$YEAR -n WTau3Mu_$CATEGORY$YEAR -M HybridNew --BDTmin 0.9900 --BDTmax 0.9910 --BDTstep 0.0005 -s all
    # compare the methods
